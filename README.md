...
1. [getCatalog](#getCatalog)
2. [getCatalogItem](#getCatalogItem)
3. [getCatalogNewest](#getCatalogNewest)
4. [getCatalogPopular](#getCatalogPopular)
5. [catalogSearch](#catalogSearch)
6. [getGenres](#getGenres)
7. [getCountries](#getCountries)
...
***
### getCatalog <a name="getCatalog"></a> 
Обычное получение каталога.  
  
Этот метод возвращает массив с объектами (элементами) каталога. Все параметры являются необязательными, а лишь вспомогательными.
#### Доступные параметры:
`page` - ID материала в базе *(default: 1)*  
`category` - Категоря на английском языке *(Один из следующих фариантов: film/serial/multfilm/multserial/anime/feature-anime)*  
`genres` - Список жанров (текстом) через запяткую, после запятой пробел не нужен *(Пример: детектив,боевик,драма)*  
`countries` - Список стран (текстом) через запяткую, после запятой пробел не нужен *(Пример: США,Канада,Уругвай)*  
`years` - Список годов (текстом) через запяткую, после запятой пробел не нужен. *(Пример: 2010,2015)*  
***
...
Получение самых просматриваемых материалов сайтаяя. API возвращает `10` материалов, которые были просмотрены больше всего раз на сайте `notalone.tv`, начиная с самого популярного. Здесь API возвращает дополнительное свойство - `visits`, показывающее количество посещений пользователями страницы материала.
#### Доступные параметры:
...
***
### getGenres <a name="getGenres"></a> 
Получение списка всех существующих в базе жанров + количества материалов с данным жанром. Количество меняется раз в сутки, поэтому нет смысла парсить этот список при каждом клике пользователя. Лучше это закешировать и изредка обновлять.  
***
### getCountries <a name="getCountries"></a> 
Получение списка всех существующих в базе стран + количества материалов с данной страной. Количество меняется раз в сутки, поэтому нет смысла парсить этот список при каждом клике пользователя. Лучше это закешировать и изредка обновлять.  
***